...
The columns that I see in this data set are OBJECTID, ZIPCODE2, ZIPNAME, EXISTING, ZIPCODE1, Shape_Leng, Shape_Area, and geometry. I see counties, cities, and airports within this data set as well.
...
The information that it contains is ZIPNAME the shape area and length of the object, the zip codes it has, where it exists and the geometery of the object we're looking at.
...
I see a map of the state of Maryland and this map has coordinates as its axis's.
...
I see a lot of data that contains the counties names, classfp, awater, geometry, MTFCC, etc. There is a lot of data that tells me about each of the counties that are displayed below.
...
I see data that looks similar to the task above, but this time it's only counties in the state of Maryland. This data set has all of the same categories as the previous one but it's all specific for Maryland counties.
...
I see a map of Maryland and the axis's are coordinates. All of the counties are grayed out on this map and when I hover over them, no data pops up which tells me this is simply just a map of Maryland for now.
...
I see a map of Maryland but this time there are lines that demonstrate as the geography of Maryland. I believe that these are county lines to show me where each county is and the length and area of them all.
...
I received two different data sets. One data set has zip codes and their foreclosure month and year starting in July 2021 and ending in March 2023. The second data set is zip codes with their forecloses but only for the month of March and only in the year 2023.
...
In this data set I can see Maryland zip codes and their March 2023 foreclosures but with this data set I can also see their geometry, their shape area and leng, their OBJECTID, and if they're existing or not.
...
I can see a map of Maryland that has different shades of blue to tell me where they stand compared to other counties in Maryland. There is a key on the right hand side that shows me which color represents what category each county falls into number wise.
...
I received two data sets in this piece of code. The first data set tells me the type of foreclosure, the date when it occurred, the county it occurred in and how many notices there were. In the second data set I can see the zip codes, the name of the counties, I can see that it was the entire population and then within that population how many people got a notice about their foreclosure.
...
In this data set I can see the zip codes, the counties, the population, and then estimate amount of people that received a notice for their foreclosure. I can also see the geometry for each county.
...
I can see the dates that their foreclosures happened, what type of foreclosure people received, what county these foreclosures happened in, and the number of notices each county had when it came to foreclosures.
...
In this data set I can see the county, the population of people, and the geometry of the county.
...
In this data set i can see the county, the population of people within the county, the date that these people received their foreclosures, the type of foreclosures people got, the number of notices people received, and the geometry of the counties listed.
...
In this data set I can see that the number of notices and population for each county has changed and increased. It increased in the number of people and notices each county now has.
...
I can see a map of Maryland that now has counties listed on it and the rate numbers have decreased.
...
I can see a map of Maryland with the same rate numbers as the previous map but this time the colors have changed to make it easier to see which county is at what rate.
...
I see a map of Maryland with the same patterns but this time the rates have changed into three categories rather than four. The regional patterns that I see are that most of the counties fall anywhere between 1.0 to 0.7357. Only one county is in the 2.0 rate while the majority fall in the purple, black, and pink colors.
