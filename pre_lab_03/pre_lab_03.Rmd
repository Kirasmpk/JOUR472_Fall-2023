...
The first warning message says "one or more parsing issues, call `problems ()` on your data frame for details, e.g.: dat \<- vroom(...). When I follows its instructions, I get a data table showing me the row number and what problem each category is facing. I can see that everyone has"a double" but the column on the far right changes variables for many of the numbers on the far left side of the table.
...
I believe that everything in this code is correct because I did not receive any parsing errors when running this code. The only thing I could think of that may be incorrect is incomplete data in some of the columns but the data looks correct to me.
...
The data type of the "Vendor Name" field is \<chr\>. The data type for the "Amount" field is \<dbl\>. The data type for the "Date" column is \<chr\>.
...
There are 549 zip codes that do not have five characters. I think since we have the less than sign before the number 5 in our code, R is showing us incomplete zip codes and how many incomplete zip codes there are in that region.
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic?

**Answer**

The thing that jumps out at me is that there are some categories that have "N/A" rather than "Grant" or "Loan" when there should be either a grant or a loan for all of the fiscal years.
...
I see that the Department of Natural Resources/Rural Legacy Program gave a grant to the Land Preservation Trust Inc. in 2009 for 60 dollars. So, for this data-set, we first see which department gave out a grant. Then we see which department received that grant and what their zip code is. Also, to add on to this we get to see a complete zip code for these departments. On the last page, we see what year this transaction occurred and how much money that grant was.
...
The headings are now lowercase rather than being uppercase.
...
There were 32 duplicates for the year 2009. There were two duplicates for the year 2020. There were two duplicates for the year 2016. There were two duplicates for the year 2017. There were four duplicates for the year 2018. There were two duplicates for the year 2019. There were two duplicates for the year 2017. There were four duplicates for the year 2021.
...
The dataframe now has 1000 rows that we are able to view.
...
The differences between the two are that the order of the zip codes changed, along with the amounts that each grant was. The years and descriptions also got changed around during this cleaning process. I can also see that in the cleaning function table that at the end of it there's a column for "Zip5" and that category is not in the data table above this one.
...
```{r}
  ...
  mutate(source = str_most_money(source), grantee = str_most_money(grantee), description  = str_to_upper(description)) |> 
  distinct() |>
  mutate(mostmoney = str_Maryland)
  mutate(zip5 = str_sub(zip_code, start=1L, end=5L)) |>
  mutate(zip5 = case_when(
    zip5 == "Vario" ~ NA,
    str_detect(zip5, "^UB7") ~ NA,
