...
**Answer It has 60712 observations or records or rows and 8 variables or columns.**
...
**Answer First line is selecting the umd_courses dataset**

the second line is taking all the terms and grouping the results by them

the third and fourth line is adding up all the records in each term in the dataset
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer I'm not really sure but what I think changed is the amount of files that are being used in this line of code. I can also see that the code has started to arrange these folders by the term and department they are in.**
...
**Answer I can tell that everything has been arranged by alphabetical order. I'm not sure if this is correct but I think the output changed by starting from the lowest value and adding it all the way up to the highest which is at the end.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer I'm not really sure what changed, but I can see that rather than doing term and department, the code arranged everything by total seats and that changed the term order and the total seat order.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer What changed was that I can now see the mean seats for the dbl but I'm not too sure what that changed. A possible answer for what changed is the tibble. The tibble went from 22 X 3 to 22 X 5. The top terms keep the same numbers all the way through and that's what they have in common.**
...
**Answer What changed was the tibble went from 22 X 5 to 212 X 5. Also the mean seats, count class, and total seats all changed to different numbers and in a different order. This affected the results by including the median seats in this dataset. The data is now sorted in order by department, count classes, total seats, mean seats, and ending with median seats.**
...
1.  When we added task 14, why was it important to have the median seats included in this dataset?

2.  Why is it that chemistry has fewer classes but more seats compared to information studies which has more classes but less seats?

3.  Why did the tibble keep switching every time I input a new but very similar piece of code?
