...
I'm not 100% sure about this answer, but I believe that this is important because now, instead of seeing instructions, seats, terms, and other categories from the previous table regarding journalism, we're able to focus on the course and the title of the course.
...
I'm not exactly sure what the correct answer is, but I think that the OR operator is giving us new results and way more results than what AND did. I think it's because the OR function, in this case, is meant to increase the values on the table.
...
The change in the expenditure date column was that instead of keeping the date as month-day-year, the new format is year-month-day.
...
I'm not 100% sure, but I think that the months are all sorted differently in this table because of the "mutate" function in the code. I believe that the table is sorted like this to show us which day out of the month had the biggest expenditures of the month.
...
I'm not totally sure about this answer, but what I think the code is doing is sorting total_votes and pct_moore into two different columns so we can see the different data. We will be able to see the total number of votes that have been cast under the total_votes column. In the pct_moore column, we will be able to see the total number of votes that were in favor of Moore.
...
I think that the answer from the previous task was changed because of how we just asked the code to arrange the data. In task 11, we could see the percentages of the votes, but it did not change how the data looked or told us. In task 12, we reordered the data to make it tell us which counties voted the most or had the highest percentage of votes for Moore. We can see which counties voted the most for Moore and which counties voted the least for him.
...
I think that this data table is a more useful answer because I can see how Moore faired against his opponents and which counties voted for him versus other candidates. I am also able to see the percentage of votes for each candidate, along with the exact number of votes they each received from each county.
...
When searching the internet for tidyverse functions similar to str_to_upper, I found that str_to_lower () is similar. This function makes letters and words lowercase. Another function I found was str_to_sentence(). This function converts everything into a sentence and makes the first letter of that sentence a capital letter.
...
I think that the mutate function is creating a new column called the "state" column within the Maryland_expenses column of the table. I also believe that the case_when function is to check addresses with the state name, and then in some way, I believe that it ties into the aberrations that are next to the state name, but I am not sure as to how or why they are mixed together.
...
This data does not make a lot of sense to me. I think I'm looking at the total number of expenditures that each state spent for the entire year, but I am not 100% sure if this is by the year or monthly.
...
1.  In task 15, why are the state abbreviations written out next to the state? What function made it do this?

2.  In task 8, why did the order of the dates switch? Was it due to the mutate function?

3.  I am still confused about the AND and OR functions. Why did the AND function keep the same number while the OR function gave us a higher number?
