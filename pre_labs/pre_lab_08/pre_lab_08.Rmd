...
It has 3822 rows and 4 columns.
...
The first line is gathering the data from the Baltimore county region and the calls for each month.

The second line is finding all the months and sorting them out.

This third line is grouping these calls by each month of the year.

The fourth line is summarizing the total amount of calls in the region that I am seeking.

The fifth line is showing us which calls were the most by descending order.

The sixth line is sorting the calls out by month.
...
I see a bar chart that has the months on the x-axis and the count of calls of the y-axis.
...
I still see the same x and y axis's but this time they're arranged from lowest total calls per month to highest total calls per month.
...
This time what I see is a flip of our x and y axis's. The total calls per month are on the y-axis while the count of calls is on the x-axis. What changed is the x and y axis.
...
What's different about this chart is that we got rid of the dead space on the left hand side and bottom of our chart. The lines are now connected to the bars rather than being connected to the gray dead space of our background. Also, the background is lighter. The font is also slightly different.
...
What has changed is there is a much different font and a blue background rather than the gray one that we've been seeing. Also, the lines are now white and not grey.
...
In this code, we now have a bolded title at the top of our bar chart, the x-axis name has been changed from "count" to "total calls" and the y-axis name has been changed from "reorder(month, total_calls)" to "month." We also have the source in the bottom right hand of our chart.
...
I see two columns that are labeled "date" and "total_calls" with 366 rows. This time rather than having just the month, we're seeing the actual date.
...
I see a massive spiky line that gives us our data. We can kind of seen our max and min amount as well.
...
I see the same spiky line, but at the bottom our dates are clumped up together because there are a lot of dates within this data.
...
Now I am seeing a wider set of our dates, but they're still clumped up slightly.
