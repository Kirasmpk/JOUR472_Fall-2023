...
In this csv file, it is a website link and that is something that I have not seen before in our pre-labs or labs.
...
9777 records. What they mean is there are 9777 data points that we can use to analyze for the information that we need. The information that we can gather from this data set is the latitude, longitude, depth, etc.
...
The depth that shows up for the deepest one is 669.9820 miles and the magnitude was 4.20.
...
There are 10 magnitudes that are larger than 6.
...
Filter with "or" (\|). There are 7,413 earthquakes that fit either these criteria.
...
You would look at the place column in the data set. There are 3,423 earthquakes in Alaska in this data set.
...
We can do a group by 'type.' The best way is to summarize it by the count_type which allows us to get the data we need. There were 5 in this data set. The two most frequent were earthquake and explosion.
...
Group and summarize with average depth. The average depth of earthquakes is 25 miles. There are also negative values.
...
We can use mutate to create a conditional column with case_when(), using the parameter of magnitude larger than 3 to make any entries as "Serious" and anything else as "Not serious".
...
I coded it to show me the hour and the count that they reached. Hour 12 has the least with 33 and hour 2 has the most with 69.
