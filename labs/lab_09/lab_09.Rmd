...
I made a bar chart because I thought that it was the easiest way to
display all of the data that I had. It made more sense to me when
looking at the data compared to all of the other charts that I had
previously looked at.

<https://datawrapper.dwcdn.net/y4VdB/1/>
...
mutate(total_votes = `Cox 2022` + `Moore 2022` + `Lashar 2022` + `Wallace 2022` + `Harding 2022` + `Sefcik 2022` + `Write-Ins 2022`, pct_Cox = (`Cox 2022`/total_votes*100))
...
The story here is the percentage change throughout the years for the
number of people who choose to do mail-in votes. Due to COVID-19 the
percent of mail-in votes skyrocketed, but after COVID-19 a good chunk of
people still choose to mail-in their votes rather than voting on the
Early day and on Election day.

<https://datawrapper.dwcdn.net/vCQlJ/1/>
...
md_votes_by_type <- md_votes_by_type |>
  mutate(total_values = Early + `Election Day` + Mail, Pct_Election_Day = (`Election Day`/total_votes)*100)
...
The story here is that Germantown, North Bethesda, and Rockville are
leading Montgomery in overdose calls in 2022. I want to know why these
areas have the highest while areas such as Poolesville have a low rate
of calls.

<https://datawrapper.dwcdn.net/aGmkL/1/>
